...
(progn
  ...
  (defparameter rooms '((lobby ((describe (if (= (get-state 'age) 7)
				...
				(east bathroom)
				(south elevator)))
			(kitchen ((describe "A typical kitchen")
				  (east lobby)))
			(ballroom ((describe "A ballroom large enough to fit a hundred people.")
				   (south lobby)))
			(bathroom ((describe "A luxurious bathroom.  Something seems odd...")
				   (west lobby)
				   (south basement)))
			(basement ((describe "The basement.  Why was the entrance hidden?")
				   (north bathroom)))
			(elevator ((describe "An elevator with three buttons for each level of the mansion")
				   (first lobby)
				   (second hallway2)
				   (third hallway3)))
			(hallway2 ((describe "The second floor hallway.")
				   (north hallway2north)
				   (east yourroom)
				   (west vacantroom1)
				   (south elevator)))
			(hallway2north ((describe "The north-side of the second floor hallway.  Huge paintings are on the sides of the walls.")
					(north yrwroom)
					(east fpbroom)
					(south hallway2)))
			(yrwroom ((describe "")
			...
			(fpbroom ((describe "")
				  (west hallway2north)))
			(yourroom ((describe "Your room.")
				   (west hallway2)))
			(vacantroom1((description "A vacant room.")
				     (east hallway2)))
			(hallway3 ((describe "The third floor hallway.  It splits into two different directions: north and east.")
				   (north hallway3north)
				   (east mcroom)
				   (west broom)
				   (south elevator)))
			(hallway3north ((describe "The north-side of the third floor hallway.")
					(east vacantroom2)
					(north storageroom)))
			(vacantroom2 ((describe "A vacant room.")
				      (west hallway3north)))
			(storageroom ((describe "A large storage room.  It looks messy.")
				      (south hallway3north)
				      (north attic)))
			(mcroom ((describe "")
				 (west hallway3)))
			(broom ((describe "")
				(east hallway3)))
			(attic ((describe "The mansion's attic.  The entrance was left open.")
				(south storageroom)))
			)))
